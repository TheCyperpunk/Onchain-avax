$filePath = "c:\Users\sangeeth karunakaran\Downloads\Onchain-avax\Onchain-SIP-main\frontend\components\Dashboard.tsx"
$lines = Get-Content $filePath
$newLines = @()
$inserted = $false

for($i=0; $i -lt $lines.Count; $i++) {
    # Check if we're at the return statement (line 161)
    if($lines[$i] -match '^\s*return \(' -and -not $inserted) {
        ...
        $newLines += '                    <h2 className="text-3xl font-bold text-white">Dashboard</h2>'
        $newLines += '                    <button'
...
Write-Host "Dashboard modal wrapper added successfully!"
